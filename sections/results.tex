...
% Then we introduce the experiments. To address the research question \textbf{Q1}, we train three tokenizers and compare them using our proposed metrics. We then pretrain masked language models using these tokenizers and see if the metrics are good predictors of the downstream performance (\textbf{Q2}). 

% With the metrics validated, we investigate the influence of the implementation, data size, alphabet size and data imbalance on the tokenization quality (\textbf{Q3}).

% Finally, we replicate the experiments of \citet{chung_improving_2020,zheng_allocating_2021,liang_xlm-v_2023},
