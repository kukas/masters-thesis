...
We focus on studying the properties of \textbf{multilingual} language models following the works of \citet{devlin_bert_2019} and \citet{conneau_unsupervised_2020}, who introduced the models mBERT\footnote{We cite the monolingual BERT paper as the mBERT model does not have an associated publication. In the literature, sometimes the mBERT Github README is cited for the multilingual variant\cite{devlin_bertmultilingualmd_2019}} and XLM-R, respectively. In these works, a Transformer \cite{vaswani_attention_2017} based multilingual masked language model is pretrained on sentences sampled from a multilingual corpus of around 100 languages. Training the models on a multilingual dataset allows the model to learn a shared representation for all languages. This shared representation can then be used for cross-lingual transfer, which the authors show on a variety of language understanding tasks.
...
The Byte Pair Encoding (BPE) algorithm was introduced by \citet{sennrich_neural_2016}. They have adapted a compression algorithm by \citet{gage_new_1994} to learn a subword vocabulary from a corpus. This method was shown to improve the performance of neural machine translation (NMT) models. BPE was subsequently used for some of the well-known pretrained models such as GPT-2 \citep{radford_improving_2018}.
...
To tokenize an input text, the BPE algorithm then iteratively merges the pairs in the same order as they were merged during training. The algorithm stops when it has merged all pairs in the vocabulary. The output of the algorithm is a sequence of subword units that can be used as the input tokens for the neural model. We show the pseudocode of the BPE tokenizer training in Algorithm \ref{alg:bpe}.
...
The implementation in the Tensorflow library follows a different, top-down approach to creating the subword vocabulary. It starts with a vocabulary of words and then iteratively splits the words into subword units to reach a target vocabulary size. \footnote{We refer the reader to the Tensorflow documentation for the Wordpiece tokenizer \url{https://www.tensorflow.org/text/api_docs/python/text/WordpieceTokenizer}}
...
\begin{figure}[ht]
    \centering
    \includegraphics[width=0.7\textwidth]{img/temp/chung_language_vectors.png}
...
With 104 languages in total, the number of clusters is set to 8. The choice of $k$ is determined empirically by running a set of preliminary experiments and comparing the results on a multilingual question-answering benchmark. The resulting clusters are shown in \autoref{tab:chung_clusters}.
...
\begin{table}[h]
    ...
    \bottomrule
    \end{tabular}
    \caption{Clusters of languages used in \cite{chung_improving_2020}. The clusters are numbered from. Table taken from \cite{chung_improving_2020}.}
    \label{tab:chung_clusters}
\end{table}
...
\begin{figure}[ht]
    \centering
    \begin{tikzpicture}[
      block/.style={rectangle, draw, fill=blue!20, text width=8.5em, text centered, rounded corners, minimum height=4em},
      line/.style={draw, -latex}
    ]
    
    \node [block] (corpora) {Monolingual Corpora};
    \node [block, right of=corpora, node distance=4.5cm] (vocab) {Train tokenizers of different sizes};
    \node [block, right of=vocab, node distance=4.5cm] (alp) {Compute ALP for each language and size};
    \node [block, below of=alp, node distance=2.5cm] (merge) {Combine tokenizers by maximizing ALP};
    \node [block, left of=merge, node distance=4.5cm] (final) {Final Vocabulary};
    
    \path [line] (corpora) -- (vocab);
    \path [line] (vocab) -- (alp);
    \path [line] (alp) -- (merge);
    \path [line] (merge) -- (final);
...
% \subsection{Commentary on the proposed methods}

% - why is it that Unigram does not work? The authors do not provide explanation, they only show that it does not work. We ask why.

% -  Rust uses mBERT which was trained with alpha=0.7

\subsection{Other tokenization approaches}
